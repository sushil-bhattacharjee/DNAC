...
devices = requests.get(url=f"{base_url}{device_endpoint}", headers=req_headers, verify=False).json()['response']
...
task_response = requests.get(
    url=f"https://10.10.20.85/dna/intent/api/v1/task/{cli_response['taskId']}", headers=req_headers, verify=False).json()
...
file = requests.get(url=f"https://10.10.20.85/dna/intent/api/v1/file/{json.loads(task_response['response']['progress'])['fileId']}", headers=req_headers, verify=False)
